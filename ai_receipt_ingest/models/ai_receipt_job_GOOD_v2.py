...
class AiReceiptJob(models.Model):
    ...
    def _get_target_move_type(self, data):
        self.ensure_one()
        document_type = (data.get('document_type') or self.document_type or 'vendor_bill').strip()
        if document_type == 'receipt':
            return 'in_receipt'
        return 'in_invoice'

    def _get_target_move_label(self, move):
        self.ensure_one()
        if move.move_type == 'in_receipt':
            return _('Purchase Receipt')
    ...
    def action_create_vendor_bill(self):
        ...
        if self.state != 'analyzed':
            raise UserError(_('Analyze the document before creating an accounting record.'))
        ...
        move_type = self._get_target_move_type(data)
    ...
    def _open_bill_action(self):
        ...
        return {
            ...
            'name': self._get_target_move_label(self.bill_id),
            'res_model': 'account.move',
