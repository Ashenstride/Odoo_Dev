...
class AiReceiptJob(models.Model):
    ...
    def _is_image_input(self):
        self.ensure_one()
        mime = (self.mime_type or '').lower()
        filename = (self.file_name or '').lower()
        return mime.startswith('image/') or filename.endswith(('.png', '.jpg', '.jpeg', '.webp', '.gif'))

    def _upload_openai_file(self, api_key, raw_bytes):
        boundary = '----OdooOpenAI%s' % uuid.uuid4().hex
        filename = self.file_name or 'document.pdf'
        body = []
        body.append(f'--{boundary}\r\n'.encode('utf-8'))
        body.append(b'Content-Disposition: form-data; name="purpose"\r\n\r\n')
        body.append(b'user_data\r\n')
        body.append(f'--{boundary}\r\n'.encode('utf-8'))
        body.append(
            (
                f'Content-Disposition: form-data; name="file"; filename="{filename}"\r\n'
                f'Content-Type: {self.mime_type or "application/octet-stream"}\r\n\r\n'
            ).encode('utf-8')
        )
        body.append(raw_bytes)
        body.append(b'\r\n')
        body.append(f'--{boundary}--\r\n'.encode('utf-8'))
        payload = b''.join(body)
        ...
        if not file_id:
            raise UserError(_('OpenAI file upload failed: no file id returned.'))
        return file_id
    ...
    def _call_openai_extract(self):
        ...
        raw_bytes = self._get_raw_file_bytes()
        contents = [{'type': 'input_text', 'text': prompt}]

        if self._is_image_input():
            ...
            file_id = self._upload_openai_file(config['api_key'], raw_bytes)
            contents.append({
                'type': 'input_file',
                'file_id': file_id,
